  ...
  return offers
    ...
    .filter(offer =>
      tenor
        ...
        tenor <= offer.curveRelativeTime.tenors[offer.curveRelativeTime.tenors.length - 1]
        : true
    )
    .filter(offer => {
      return sell
        ? Number(offer.user.borrowATokenBalance) / 10 ** tokens.BorrowAToken.decimals > amount
        : price
          ? collateralRatio(tokens, offer, price, amount) > Math.max(Number(offer.user.user.openingLimitBorrowCR) / 1e18, 1.5)
          : false
    }
    );
}
