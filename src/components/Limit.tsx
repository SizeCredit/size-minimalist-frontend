...
const actions = ["BuyCreditLimit", "SellCreditLimit"];
...
const Limit = () => {
  ...
  const [days, setDays] = useState(
    offer.curveRelativeTime.tenors.map((e) =>
      (Number(e) / (60 * 60 * 24)).toString(),
    ),
  );
  const [aprs, setAprs] = useState(
    offer.curveRelativeTime.aprs.map((e) => (100 * Number(e)) / 1e18),
  );
  ...
  const onClick = () => {
    if (action === actions[0]) {
      buyCreditLimit(
        days.map((e) => BigInt(Number(e) * 60 * 60 * 24)),
        aprs.map((e) => BigInt((e * 1e18) / 100)),
      );
    } else {
      sellCreditLimit(
        days.map((e) => BigInt(Number(e) * 60 * 60 * 24)),
